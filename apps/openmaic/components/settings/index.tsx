...
export function SettingsDialog({ open, onOpenChange, initialSection }: SettingsDialogProps) {
  ...
  // Reconcile the dialog's local navigation with provider selections that may
  // change during the asynchronous server-provider sync. Without this, a
  // freshly configured managed provider can appear in the list while the
  // detail panel remains stuck on the pre-sync default.
  useEffect(() => {
    if (!open) return;
    // eslint-disable-next-line react-hooks/set-state-in-effect -- Sync dialog navigation from persisted/server-managed selections
