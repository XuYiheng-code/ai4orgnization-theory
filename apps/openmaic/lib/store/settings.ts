...
const MANAGED_TTS_SETUP_VERSION = 1;
...
export interface SettingsState {
  ...
  // One-time local-deployment bootstrap for a TTS provider that becomes
  // server-managed after the browser has already completed initial setup.
  managedTtsSetupVersion: number;
  ...
  persist(
        ...
        fetchServerProviders: async () => {
          try {
            ...
            set((state) => {
              ...
              const shouldBootstrapManagedTts =
                serverTtsIds.length > 0 &&
                state.managedTtsSetupVersion < MANAGED_TTS_SETUP_VERSION;
              ...
              // This deployment may add its managed TTS credential after a
              // browser has already completed the generic first-run sync. In
              // that case, perform one setup pass so stale "no voice" state
              // cannot survive the newly available provider. The version is
              // persisted, so later user choices are never overwritten.
              if (shouldBootstrapManagedTts) {
                autoTtsProvider = serverTtsIds[0];
                autoTtsVoice =
                  DEFAULT_TTS_VOICES[autoTtsProvider as BuiltInTTSProviderId] || 'default';
                autoTtsEnabled = true;
              ...
              return {
                ...
                managedTtsSetupVersion: shouldBootstrapManagedTts
                  ? MANAGED_TTS_SETUP_VERSION
                  : state.managedTtsSetupVersion,
    ...
    },
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        if ((state as Record<string, unknown>).managedTtsSetupVersion === undefined) {
          (state as Record<string, unknown>).managedTtsSetupVersion = 0;
        }
