...
const QUOTA_EXHAUSTED_PATTERN =
  /free quota exhausted|quota exhausted|insufficient[_ ]quota|add funds|free tier only|billing quota|credit balance/i;

function hasQuotaExhaustedMessage(error: unknown, seen = new Set<unknown>()): boolean {
  if (!error || seen.has(error)) return false;
  seen.add(error);
  if (error instanceof Error && QUOTA_EXHAUSTED_PATTERN.test(error.message)) return true;
  if (!isRecord(error)) return false;
  const direct = [error.message, error.error, error.details].some(
    (value) => typeof value === 'string' && QUOTA_EXHAUSTED_PATTERN.test(value),
  );
  if (direct) return true;
  return (
    hasQuotaExhaustedMessage(error.cause, seen) ||
    hasQuotaExhaustedMessage(error.lastError, seen) ||
    (Array.isArray(error.errors) &&
      error.errors.some((nested) => hasQuotaExhaustedMessage(nested, seen)))
  );
}
...
export function llmApiError(error: unknown) {
  ...
  if (hasQuotaExhaustedMessage(error)) {
    return apiError(
      'QUOTA_EXHAUSTED',
      status === 402 || status === 429 ? status : 429,
      'The configured model quota is exhausted. Add quota or switch models, then retry.',
    );
  }
  if (status === undefined) {
