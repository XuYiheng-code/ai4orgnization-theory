...
} from '@/lib/generation/client-retry';
import {
  completedSceneOrders,
  writeGenerationRecovery,
  type GenerationFailurePhase,
  type GenerationRecoveryRecord,
} from '@/lib/generation/recovery-store';

const log = createLogger('SceneGenerator');

function phaseLabel(phase: GenerationFailurePhase): string {
  switch (phase) {
    case 'content':
      return '内容生成';
    case 'actions':
...
function createHttpError(
  ...
  fallback: string,
): Error & { errorCode?: string; statusCode?: number; isRetryable?: boolean } {
  const message =
  ...
  if (typeof data.errorCode === 'string') {
    ...
    if (data.errorCode === 'QUOTA_EXHAUSTED') error.isRetryable = false;
  }
...
export function getPendingOutlines(
...
export function findCheckpointResumeOutline({
  outlineId,
  failedOutlines,
  outlines,
  scenes,
  ...
  const pending = getPendingOutlines(outlines, scenes);
  if (outlineId) {
    return (
      failedOutlines.find((outline) => outline.id === outlineId) ??
      pending.find((outline) => outline.id === outlineId) ??
      outlines.find(
        (outline) =>
          outline.id === outlineId && !scenes.some((scene) => scene.order === outline.order),
      )
    );
  }
  return failedOutlines[0] ?? pending[0];
}
...
export interface UseSceneGeneratorOptions {
  ...
  onComplete?: () => void;
  onRecoveryChange?: (record: GenerationRecoveryRecord) => void;
}
...
export function useSceneGenerator(options: UseSceneGeneratorOptions = {}) {
  ...
  const saveRecovery = useCallback(
    (
      status: GenerationRecoveryRecord['status'],
      params: GenerationParams,
      currentOutlineId?: string,
      failure?: GenerationRecoveryRecord['failure'],
    ) => {
      const state = store.getState();
      if (!state.stage) return null;
      const record = writeGenerationRecovery({
        stageId: state.stage.id,
        status,
        params,
        completedOrders: completedSceneOrders(state.scenes),
        totalOutlines: state.outlines.length,
        currentOutlineId,
        failure,
      });
      if (record) options.onRecoveryChange?.(record);
      return record;
    },
    [options, store],
  );

  const saveFailure = useCallback(
    (
      params: GenerationParams,
      outline: SceneOutline,
      phase: GenerationFailurePhase,
      result: { error?: string; errorCode?: string; statusCode?: number },
      fallback: string,
    ) => {
      const message = result.error || fallback;
      saveRecovery('failed', params, outline.id, {
      ...
      // Localized, actionable failure surface. Historically the API's raw
      // English message was shown verbatim — confusing for a zh-CN user and
      // leaving them without a clear next step.
      const description = (() => {
        if (result.errorCode === 'QUOTA_EXHAUSTED') {
          return t('stage.generationResumeToastQuota');
        }
        if (
          ...
          return t('stage.generationResumeToastCredentials');
        }
        if (result.errorCode === 'RATE_LIMITED' || result.statusCode === 429) {
          return t('stage.generationResumeToastRateLimit');
        }
        if (phase === 'persistence') {
          return t('stage.generationResumeToastPersistence');
        }
        return `${t('stage.generationResumeToastGeneric')} (${phaseLabel(phase)}: ${message})`;
      })();
      toast.error(t('stage.generationResumeToastTitle'), {
        description,
        duration: 10000,
      });
    },
  ...
  const generateRemaining = useCallback(
    async (params: GenerationParams) => {
      ...
      saveRecovery('generating', params);
      ...
      if (pending.length === 0) {
        ...
        if (await store.getState().saveToStorage()) saveRecovery('completed', params);
        options.onComplete?.();
        generatingRef.current = false;
        return;
      }
      ...
      try {
        ...
        for (const outline of pending) {
          ...
          saveRecovery('generating', params, outline.id);
          ...
          if (!contentResult.success || !contentResult.content) {
            ...
            saveFailure(params, outline, 'content', contentResult, 'Content generation failed');
            if (contentPromises) {
          ...
          if (actionsResult.success && actionsResult.scene) {
            ...
            if (
              ...
              if (!ttsResult.success) {
                ...
                saveFailure(params, outline, 'tts', ttsResult, 'TTS generation failed');
                store.getState().setGenerationStatus('paused');
                pausedByFailureOrAbort = true;
                break;
              }
            ...
            const durable = await store.getState().saveToStorage();
            if (!durable) {
              store.getState().addFailedOutline(outline);
              store.getState().setGenerationStatus('paused');
              saveFailure(
                ...
                outline,
                'persistence',
                { error: 'The generated scene could not be saved locally.' },
                'The generated scene could not be saved locally.',
              );
              pausedByFailureOrAbort = true;
              break;
            }
            ...
            saveFailure(params, outline, 'actions', actionsResult, 'Actions generation failed');
            store.getState().setGenerationStatus('paused');
            pausedByFailureOrAbort = true;
            break;
          }
        ...
        if (isAbortError(err)) {
          ...
          store.getState().setGenerationStatus('paused');
          saveRecovery('paused', params, store.getState().generatingOutlines[0]?.id);
        } else {
          const outline = store.getState().generatingOutlines[0];
          if (outline) {
            store.getState().addFailedOutline(outline);
            saveFailure(
              params,
          ...
          store.getState().setGenerationStatus('paused');
          log.error('Generation pipeline failed:', err);
        }
      } finally {
        generatingRef.current = false;
    ...
    [options, saveFailure, saveRecovery, store],
  );
  ...
  const stop = useCallback(() => {
    ...
    if (params && store.getState().stage) {
      saveRecovery('paused', params, store.getState().generatingOutlines[0]?.id);
    }
  }, [saveRecovery, store]);
  ...
  const setResumeParams = useCallback((params: GenerationParams) => {
    lastParamsRef.current = params;
  }, []);
  ...
  const retrySingleOutline = useCallback(
    async (outlineId: string) => {
      const state = store.getState();
      const params = lastParamsRef.current;
      if (!state.stage || !params) return;
      const outline = findCheckpointResumeOutline({
        outlineId,
        failedOutlines: state.failedOutlines,
        outlines: state.outlines,
        scenes: state.scenes,
      });
      if (!outline) {
        await generateRemainingRef.current?.(params);
        return;
      }
      ...
      try {
        ...
        const pendingAfterRetry = getPendingOutlines(latestState.outlines, latestState.scenes);
        if (pendingAfterRetry.length > 0 && lastParamsRef.current) {
          ...
          if (store.getState().generationComplete && (await store.getState().saveToStorage())) {
            saveRecovery('completed', params);
          }
        }
      } catch (err) {
        if (!isAbortError(err)) {
    ...
    [saveFailure, saveRecovery, store],
  );

  return { generateRemaining, retrySingleOutline, setResumeParams, stop, isGenerating };
}
