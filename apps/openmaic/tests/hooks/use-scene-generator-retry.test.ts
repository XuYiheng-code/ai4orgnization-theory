...
describe('browser scene generation retry wrappers', () => {
  ...
  it('does not waste retries after the provider reports exhausted quota', async () => {
    const { fetchSceneContent } = await import('@/lib/hooks/use-scene-generator');
    mockFetch.mockResolvedValue(
      jsonResponse(429, {
        success: false,
        errorCode: 'QUOTA_EXHAUSTED',
        error: 'The configured model quota is exhausted.',
      }),
    );
  ...
  it('finds the checkpoint outline even when failedOutlines was not restored', async () => {
    const { findCheckpointResumeOutline } = await import('@/lib/hooks/use-scene-generator');
    ...
    const scenes = outlines.slice(0, 8).map((item) => ({
      id: `scene-${item.order}`,
      stageId: 'stage-1',
      type: 'slide',
      title: item.title,
      order: item.order,
      content: { type: 'slide', canvas: { id: `canvas-${item.order}`, elements: [] } },
      actions: [],
    }));

    const resume = findCheckpointResumeOutline({
      outlineId: 'outline-9',
      failedOutlines: [],
      outlines,
    ...
    expect(resume?.id).toBe('outline-9');
  });

  it('falls back to the first missing outline when no explicit checkpoint is available', async () => {
    const { findCheckpointResumeOutline } = await import('@/lib/hooks/use-scene-generator');
    const outlines = [
      { ...outline, id: 'outline-1', order: 1 },
      { ...outline, id: 'outline-2', order: 2 },
      { ...outline, id: 'outline-3', order: 3 },
    ];
    const scenes = [
      {
        id: 'scene-1',
    ...
    expect(resume?.id).toBe('outline-2');
  });
});
