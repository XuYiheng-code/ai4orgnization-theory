...
function HomePage() {
  ...
  // ─── Publish to Knowledge Plaza (course-platform-prototype on 8765) ──
  // 把当前 OpenMAIC 课堂作为作品发布到知识广场的「项目课程平台」，
  // 让同学之间可以互相看到、点赞、评论。本调用走跨域 fetch。
  // 发布地址解析：部署可通过 window.__PLAZA_IMPORT_URL 覆盖（如 Nginx 同域反代）。
  // 本地开发(127.0.0.1)：OpenMAIC(3100) 跨域调 课程平台(8765)。
  // 线上部署：OpenMAIC 与知识广场同域时改用相对路径，远程同学也能发布。
  const resolvePlazaImportUrl = (): string => {
    const override = typeof window !== 'undefined' &&
      (window as unknown as { __PLAZA_IMPORT_URL?: string }).__PLAZA_IMPORT_URL;
    if (override) return override;
    const host = typeof window !== 'undefined' ? window.location.hostname : 'localhost';
    const isLocal = host === 'localhost' || host === '127.0.0.1';
    return isLocal ? 'http://127.0.0.1:8765/api/plaza/import' : '/api/plaza/import';
  };

  // 从富文本 / canvas 文本元素中提取纯文本
  ...
  // 把 OpenMAIC stage 简化为课程平台可消费的 slides 快照（真实内容）
  const buildSlidesSnapshot = async (stageId: string) => {
    try {
      const data = await loadStageData(stageId);
      const scenes: any[] = (data as any)?.scenes ?? (data as any)?.document?.scenes ?? [];
      return scenes.slice(0, 30).map((s: any, i: number) => {
        const c: any = s.content ?? {};
        // 标题优先取场景自身 title，再回退到旧格式字段
        const title =
          s.title ??
          c?.title ??
          c?.heading ??
          ...
          `场景 ${i + 1}`;
        // 正文：优先从 canvas.elements 的文本元素抽取（真实课程结构），
        // 回退到旧格式的 text / description / script
        let text = '';
        ...
        if (!text) {
          text =
            (typeof c === 'string' ? c : c?.text) ?? c?.description ?? c?.script ?? '';
        }
        return {
      ...
      log.error('Failed to load stage data for publish:', err);
      return [];
    }
  };

  const publishOne = async (classroom: StageListItem) => {
    const nickname = useUserProfileStore.getState().nickname || '学习者';
    const slides = await buildSlidesSnapshot(classroom.id);
    const payload = {
      stageId: classroom.id,
      title: classroom.name,
      description: classroom.description ?? '',
      sceneCount: classroom.sceneCount ?? slides.length,
      createdAt: classroom.createdAt ?? Date.now(),
      ownerName: nickname,
      ownerAvatar: nickname.slice(0, 1),
      slides,
    };
    const res = await fetch(resolvePlazaImportUrl(), {
      method: 'POST',
      // 携带课程平台会话凭证，使服务端能把作品归属到登录账户（ownerUserId）。
      credentials: 'include',
  ...
  const handlePublishToPlaza = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const classroom = classrooms.find((c) => c.id === id);
    if (!classroom) return;
    try {
      const data = await publishOne(classroom);
      if (data.duplicate) {
        toast.success(t('classroom.publishDuplicate'));
      } else {
        toast.success(t('classroom.publishSuccess'));
      }
    } catch (err) {
      log.error('Failed to publish to plaza:', err);
      toast.error(t('classroom.publishFailed'));
    }
  };
  ...
  const handleBatchPublish = async () => {
    ...
    const selected = classrooms.filter((c) => selectedIds.has(c.id));
    if (selected.length === 0) return;
    const toastId = toast.loading(`正在发布 ${selected.length} 个课程到知识广场…`);
    let success = 0;
    let duplicate = 0;
    let failed = 0;
    for (const classroom of selected) {
      try {
        ...
        log.error('Batch publish failed for', classroom.id, err);
        failed += 1;
      }
    }
    toast.dismiss(toastId);
    if (failed === 0) {
      toast.success(`已发布 ${success} 个${duplicate > 0 ? `，${duplicate} 个已存在` : ''} → 知识广场`);
    } else {
      toast.warning(`已发布 ${success}，跳过 ${duplicate}，失败 ${failed}`);
    }
                      ...
                      {selectedIds.size > 0 && (
                        ...
                        >
                          ...
                          <div className="ml-auto flex items-center gap-2">
                            ...
                            <Button
                              size="sm"
                              className="text-[12px] h-7 bg-emerald-600 hover:bg-emerald-500 text-white"
                        ...
                        {visibleClassrooms.map((classroom, i) => (
                          ...
                          >
                            <ClassroomCard
                              ...
                              onPublish={(id, e) => handlePublishToPlaza(id, e)}
                              selected={selectedIds.has(classroom.id)}
                              onToggleSelected={() => toggleSelected(classroom.id)}
...
function ClassroomCard({
  ...
  /** 发布到课程平台的知识广场（跨域 fetch 到 8765）。*/
  onPublish?: (id: string, e: React.MouseEvent) => void;
  /** 是否被勾选（多选发布）。*/
  selected?: boolean;
  /** 切换勾选状态。*/
  onToggleSelected?: () => void;
  /** 至少有一张卡片被勾选时显示 checkbox（避免空操作）。*/
  ...
  return (
    ...
    >
      ...
      <div
        ref={thumbRef}
        className={cn(
          'relative w-full aspect-[16/9] rounded-2xl bg-slate-100 dark:bg-slate-800/80 overflow-hidden transition-transform duration-200 group-hover:scale-[1.02]',
          selected && 'ring-2 ring-emerald-500 ring-offset-2 ring-offset-background',
        )}
      >
        {/* 多选 checkbox（仅在批量模式下显示） */}
        {showCheckbox && (
          <label
            className="absolute top-2 left-2 z-20 size-7 rounded-md bg-white/90 dark:bg-slate-900/80 backdrop-blur-sm shadow-sm flex items-center justify-center cursor-pointer hover:bg-white"
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      size="icon"
                      variant="ghost"
                      className="absolute top-2 right-[68px] size-7 opacity-0 group-hover:opacity-100 transition-opacity bg-emerald-500/70 hover:bg-emerald-500 text-white hover:text-white backdrop-blur-sm rounded-full"
                      onClick={(e) => {
                        e.stopPropagation();
                        onPublish(classroom.id, e);
                      }}
                    >
